...
func getEmbedAlert(Performances []monitor.Performance) []Embed {
	...
	for _, Performance := range Performances {
		if Performance.Sync.Synced == false {
			if Performance.Sync.Error == "" {
				embed := Embed{
					Title:       "Celestia Light Node Sync Alert",
					Description: "*Your light node sync check show some problems, please check it*",
					Color:       16711680,
					Fields: []*EmbedField{
						{
							Name:   "Gateway API",
						...
						{
							Name:   "Error",
							Value:  "Your light node not synced",
							Inline: false,
						},
						{
							Name:   "Time",
		...
		if Performance.MinBalance.Enough == false {
			if Performance.MinBalance.Error == "" {
				embed := Embed{
					Title:       "Celestia Light Node Min balance check Alert",
					Description: "*Your light node minimum balance check found some problem, please check it*",
					Color:       16711680,
					Fields: []*EmbedField{
						{
							Name:   "Gateway API",
						...
						{
							Name:   "Error",
							Value:  "Your light node balance is lower than minimum balance you set",
							Inline: false,
						},
						{
							Name:   "Time",
